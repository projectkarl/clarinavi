...
async function build(code, months){
  ...
  if(!snapshot)throw new Error(`找不到股票代號，或公開資料源暫時無法連線${sourceErrors.length?`（${sourceErrors.join('；')}）`:''}`);

  const [valuation, historyRaw] = await Promise.all([
    getValuation(code,snapshot.market),
    snapshot.market==='TWSE'?getTwseHistory(code,months):getTpexHistory(code,months),
  ]);
...
module.exports=async function handler(req,res){
  ...
  try{const data=await build(code,months);cache.set(key,{time:Date.now(),data});return res.status(200).json(data);}catch(e){return res.status(502).json({error:e.message||'資料讀取失敗'});}
};
